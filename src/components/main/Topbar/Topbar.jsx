...
const Topbar = () => {
  ...
  return (
    <div className="border-b border-border h-16 shrink-0 top-0 sticky bg-background/90 backdrop-blur-sm  w-full p-4 flex items-center">
      <div
        className={`text-xl font-sans sm:block hidden font-bold ${
          theme === "dark" ? "text-primary" : "text-primary-foreground"
        }`}
      >
        Home
      </div>
      <div className="flex gap-2 ml-auto items-stretch w-full justify-end">
        <div className="relative group w-full sm:w-fit ">
          <Search
            size={14}
            className="absolute pointer-events-none transition-all left-3 group-focus-within:text-primary top-1/2 transform -translate-y-1/2 text-gray-500 z-10"
          />
          <Input
            className={"sm:min-w-[300px] sm:w-[300px] w-full pl-8"}
